...
class FunctionalTest {
    ...
    fun `test simple test project`(@TempDir(cleanup = CleanupMode.ON_SUCCESS) tempDir: Path) {
        ...
        assertEquals(TaskOutcome.SUCCESS, rebP2.task(":test-server:rebuildBasePatches")?.outcome)
        assertEquals(
        ...
        val rebP4 = gradleRunner
            ...
            .withDebug(debug)
            .build()
        assertEquals(TaskOutcome.SUCCESS, rebP4.task(":test-server:rebuildPatches")?.outcome)
        assertEquals(
